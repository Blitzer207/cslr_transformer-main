...
def tokenize(dataset_split: str):
    """
    ...
    full_text = None
    NAME = "info_multi.npy" if MULTISINGER else "info_si5.npy"
    if dataset_split != "train" and dataset_split != "dev" and dataset_split != "test":
        raise ValueError(
            "Please give a valid dataset split. This is either train, dev or test"
        )
    else:
        dataset_split = "/" + dataset_split + "_"
        data = np.load(CURRENT + dataset_split + NAME, allow_pickle=True)
        full_text = " ".join(s[-1] for s in data)
    tokens = word_tokenize(full_text, language="german")
    freq_distribution = FreqDist(tokens)
    return tokens, freq_distribution


def tokenize_all(save: bool = False):
    """
    ...
    train = np.load(CURRENT + "/train_" + NAME, allow_pickle=True)
    test = np.load(CURRENT + "/test_" + NAME, allow_pickle=True)
    dev = np.load(CURRENT + "/dev_" + NAME, allow_pickle=True)
    full_train = " ".join(s[-1] for s in train)
    full_dev = " ".join(s[-1] for s in dev)
    full_test = " ".join(s[-1] for s in test)
    full = full_train + " " + full_dev + " " + full_test
    tokens = word_tokenize(full, language="german")
    freq_distribution = FreqDist(tokens)
